...
import { cognitiveTest } from "@/lib/data/screenings";
...
export function CognitiveTest() {
  ...
  const currentQuestion = cognitiveTest.questions[currentIndex];
  const totalQuestions = cognitiveTest.questions.length;
  ...
  const scoresByDomain = useMemo(() => {
    ...
    cognitiveTest.questions.forEach((q) => {
  ...
  const interpretation = useMemo(() => {
    if (totalScore >= 25) {
      return "정상 범위로 보입니다. 꾸준한 인지 활동을 유지하세요.";
    }
    if (totalScore >= 20) {
      return "경도의 인지 변화가 관찰될 수 있습니다. 전문가 상담을 권장합니다.";
    }
    if (totalScore >= 15) {
      return "중등도의 관심이 필요합니다. 반드시 전문가와 상담하세요.";
    }
    return "전문가 상담이 권장됩니다. 의료 기관을 방문해주세요.";
  }, [totalScore]);
  ...
  useEffect(() => {
    ...
    const onGesture = () => {
      ...
      speak(`${cognitiveTest.title}. ${cognitiveTest.questions.length}문항이며, 예상 소요 시간은 ${estimatedMinutes}분입니다.`, { key: "intro", force: true });
  ...
  if (phase === "intro") {
    return (
      <div className="space-y-4">
        <div className="rounded-2xl border border-slate-200 bg-white p-6 space-y-3">
          <h1 className="text-2xl font-semibold">{cognitiveTest.title}</h1>
          <p className="text-slate-700">{cognitiveTest.description}</p>
          <div className="rounded-xl bg-amber-50 border border-amber-200 p-4 text-sm text-slate-800">
            ...
            <div className="mt-1">{cognitiveTest.disclaimer}</div>
          </div>
          <div className="flex flex-wrap gap-4 text-sm text-slate-700">
            ...
            <div>• 배점: 총 <span className="font-semibold">{cognitiveTest.maxScore}점</span></div>
          </div>
        </div>
    ...
    return (
      <div className="space-y-4">
        <div className="rounded-2xl border border-brand-200 bg-brand-50 p-6 space-y-3">
          ...
          <div className="text-3xl font-bold text-brand-800">{totalScore} / {cognitiveTest.maxScore}점</div>
